...
func New(config *ManagerConfig) Manager {
	return &manager{
		...
		unblockChainCreatorCh:  config.UnblockChainCreatorCh,
...
func (m *manager) AddChain(chain *Chain) {
	...
	chain.Handler.Start(context.TODO(), !m.CriticalChains.Contains(chain.Context.XChainID))
}

// New returns a new chain
func (f *Factory) New(
	chainID, subnetID, vmID ids.ID,
	fxIDs []ids.ID,
	genesis []byte,
